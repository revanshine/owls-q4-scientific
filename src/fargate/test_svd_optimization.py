...
for method, result in results.items():
    ...
    print(
        f"{method:>12} | {result['time']:>6.3f}s | {max_diff:>8.2e} | {mean_diff:>8.2e}"
    )
...
print("   • TruncatedSVD: O(ndk) vs O(n²d) for full SVD")
print("   • Randomized: O(ndk + k³) - best for large datasets")
print("   • Auto-selection based on data size")
...
mean_diff_large = np.mean(
    [abs(a - b) for a, b in zip(randomized_large, baseline_large)]
)

print(f"   Max difference: {max_diff_large:.2e}")
print(f"   Mean difference: {mean_diff_large:.2e}")
print(
    f"   Speed improvement: {stress_results['truncated']['time'] / stress_results['randomized']['time']:.2f}x"
)
