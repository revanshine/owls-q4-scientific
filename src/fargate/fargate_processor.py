...
def process_q4_scientific_fargate(input_key, run_prefix, processing_mode="standard"):
    ...
    try:
        ...
        print(
            f"✅ Dataset size validation passed: {X.shape[0]} vectors (limit: {MAX_VECTORS})"
        )
        ...
        if processing_mode in ["enhanced", "full"]:
            ...
            svd_model = fit_svd(
                Vn, k=k, seed=42, method="auto"
            )  # Use optimized auto-selection
            svd_model.mu = mu
            ...
            svd_analysis = {
                ...
                "explained_variance_ratio": (
                    svd_model.explained_variance / svd_model.explained_variance.sum()
                ).tolist()[:10],
        ...
        if processing_mode == "full" and Z is not None:
            ...
            try:
                ...
                artifacts = embed_reduce_heatmap(
                    qstudy_df, plot_dir, reducer="pca", bins=40
                )
        ...
        if hasattr(q_result, "Q_discard") and q_result.Q_discard.size > 0:
            ...
            s3_client.put_object(
                Bucket=artifacts_bucket, Key=discard_key, Body=discard_csv
            )
        ...
        print("✅ Scientific Q4 Fargate processing completed successfully")
...
def send_completion_notification(manifest, run_id, input_key, processing_mode):
    ...
    try:
        ...
        sqs_client.send_message(
            QueueUrl=done_queue_url, MessageBody=json.dumps(completion_message)
        )
