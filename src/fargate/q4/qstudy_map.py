...
def embed_reduce_heatmap(
    qstudy_df: pd.DataFrame, out_dir: Path, reducer: str = "pca", bins: int = 40
):
    ...
    if reducer.lower() == "umap":
        try:
            ...
            X2 = umap.UMAP(n_neighbors=15, min_dist=0.1, n_components=2).fit_transform(
                X
            )
        except Exception:
    ...
    sc = plt.scatter(
        qstudy_df["e0"], qstudy_df["e1"], c=qstudy_df["frac_tail_mean"], s=30
    )
    plt.colorbar(sc, label="frac_tail_mean")
    plt.xlabel("embed-0")
    plt.ylabel("embed-1")
    plt.title("Q_study embedding")
    plt.tight_layout()
    plt.savefig(scatter_path)
    plt.close()

    H_count, xedges, yedges = np.histogram2d(
        qstudy_df["e0"], qstudy_df["e1"], bins=bins
    )
    H_val, _, _ = np.histogram2d(
        qstudy_df["e0"],
        qstudy_df["e1"],
        bins=[xedges, yedges],
        weights=qstudy_df["frac_tail_mean"],
    )
    ...
    plt.xlabel("embed-0")
    plt.ylabel("embed-1")
    plt.title("Q_study anisotropy heatmap (mean frac_tail)")
    plt.tight_layout()
    ...
    return {
        ...
        "embedding_csv": str(out_dir / "qstudy_embedding.csv"),
    }
