    ...
    def test_svd_model_save_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_array_almost_equal(
                model.components, loaded_model.components
            )
            np.testing.assert_array_almost_equal(
                model.explained_var, loaded_model.explained_var
            )
...
class TestAnisotropyFixed:
    ...
    def test_anisotropy_extreme_cases(self):
        ...
        aniso_iso = Decimal(str(aniso_iso_raw)).quantize(
            Decimal("0.0001"), rounding=ROUND_HALF_UP
        )
        aniso_dir = Decimal(str(aniso_dir_raw)).quantize(
            Decimal("0.0001"), rounding=ROUND_HALF_UP
        )
